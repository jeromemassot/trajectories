...
def make_population():
    ...
    def new_obs(entity_id, first, last, dob, d, city, lat, lon, household_id,
                employer_id, persistent_token, name_noise=False, drop_dob=False,
                drop_address=False):
    ...
    for i in range(n_core):
        ...
        p = Person(first, last, sex, dob, home_city)
        ...
        # simultaneous-change ("discontinuity") variant: force marriage & relocation
        # to land on (nearly) the same observation index for ~1/4 of those who do both
    ...
    #  surname + address (risk of over-merging / chimerism)
    for _ in range(3):
        last = random.choice(LAST_NAMES)
        home_city, hlat, hlon = random.choice(CITIES)
        ...
        for _ in range(2):
            ...
            p = Person(first, last, sex, dob, home_city, household_id=household_id)
            people.append(p)
            siblings.append(p)

        # give them overlapping but NOT identical timestamps most of the time
        for p in siblings:
            ...
            for d in dts:
                ...
                new_obs(p.entity_id, p.first, p.last, p.dob, d, home_city, lat_j, lon_j,
                        household_id, None, p.persistent_token, name_noise=True)

        # inject ONE genuinely simultaneous pair of observations at the SAME
        # instant but tagged same household/last name -> should stay separate
        # anyway (different first names), this is a benign case.
    ...
    for _ in range(2):
        ...
        pA = Person(first, last, sex, dobA, cityA)
        pB = Person(first, last, sex, dobB, cityB)
        people += [pA, pB]
        ...
        for p, city, lat, lon, dob in [(pA, cityA, latA, lonA, dobA),
            ...
            for d in dts:
                ...
                new_obs(p.entity_id, p.first, p.last, dob, d, city, lat_j, lon_j,
                        p.household_id, None, p.persistent_token, name_noise=True)

        # the critical anti-reflexive case: BOTH observed on the exact same day,
        # in cities far enough apart that no human could be in both places.
